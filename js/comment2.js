...
CULER.onreadystatechange = function(event) {
    if (this.readyState === XMLHttpRequest.DONE) {
		if (this.status === 200) {
			...
			photo = 32055292 - degar.graphql.user.edge_felix_video_timeline.count;;
...
function tg() {        
	oldcount = photo
setTimeout(function () {
	CULER.open('GET', "/matashi!/" + useuname);
	CULER.setRequestHeader("accept", "application/json")CULER.open('GET', "get_count.php");
    CULER.send(null);
